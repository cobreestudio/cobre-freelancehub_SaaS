...
export default async () => {
  ...
  for (const invoice of invoices) {
    ...
    if (!emailError) {
      ...
      sent++
      console.log(`Reminder sent to ${client.email} for invoice ${invoice.id}`)
    } else {
      console.error(`Failed to send to ${client.email}:`, emailError)
    }
  }

  console.log(`Done: ${sent}/${invoices.length} reminders sent`)
  return new Response(JSON.stringify({ sent, total: invoices.length }))
}
