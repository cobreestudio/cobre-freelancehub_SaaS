...
import { Crown, Check, Zap, ArrowLeft, AlertCircle } from 'lucide-react'
...
export default function BillingPage() {
  ...
  useEffect(() => {
    ...
    profileStore.get().then(p => { setPlan(p?.plan || 'free'); setLoading(false) })
  }, [])
    ...
    <div className="max-w-2xl">
      ...
      {success && (
        <div className="bg-emerald-50 border border-emerald-200 rounded-2xl px-5 py-4 flex items-center gap-3 mb-6">
          ...
          <p className="text-sm font-semibold text-emerald-800">{t('successMsg')}</p>
        </div>
      )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className={`bg-white rounded-2xl border-2 p-6 flex flex-col ${plan === 'free' ? 'border-indigo-300' : 'border-gray-100'}`}>
            <div className="flex items-center justify-between mb-1">
              <h2 className="font-bold text-gray-900">{t('free')}</h2>
              {plan === 'free' && (
                <span className="text-xs font-semibold bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded-full">{t('currentPlan')}</span>
              )}
            </div>
            <p className="text-3xl font-extrabold text-gray-900 mb-4">
              0 €<span className="text-base font-medium text-gray-400">{t('perMonth')}</span>
            </p>
          ...
          <div className={`rounded-2xl border-2 p-6 flex flex-col ${plan === 'pro' ? 'bg-indigo-600 border-indigo-600 text-white' : 'bg-white border-indigo-300'}`}>
            <div className="flex items-center justify-between mb-1">
              ...
              {plan === 'pro' && (
                <span className="text-xs font-semibold bg-white/20 text-white px-2 py-0.5 rounded-full">{t('currentPlan')}</span>
              )}
            </div>
            <p className={`text-3xl font-extrabold mb-4 ${plan === 'pro' ? 'text-white' : 'text-gray-900'}`}>
              12 €<span className={`text-base font-medium ${plan === 'pro' ? 'text-indigo-200' : 'text-gray-400'}`}>{t('perMonth')}</span>
            </p>
            <ul className="space-y-2 flex-1">
              {proFeatures.map(f => (
            ...
            {plan === 'free' ? (
              ...
              <button onClick={handleManage} disabled={upgrading}
                ...
                {upgrading ? t('loading') : t('manageSubscription')}
              </button>
            )}
          </div>
        </div>
      ...
      <p className="text-xs text-gray-400 mt-5 text-center">{t('securePayments')}</p>
    </div>
  )
}
