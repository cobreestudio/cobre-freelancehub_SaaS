...
export default function ProfilePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(false)
    show('Perfil guardado correctamente')
  }
  ...
  return (
    <div className="max-w-2xl">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-400 text-sm mt-0.5">Estos datos aparecerán en tus facturas PDF</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <section className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          <div className="flex items-center gap-2 mb-5">
            ...
            <h2 className="font-semibold text-gray-800">Datos de negocio</h2>
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              <label className="block text-sm font-medium text-gray-700 mb-1.5">
                <span className="flex items-center gap-1.5"><Building size={13} />Nombre del negocio / Autónomo</span>
              </label>
              <input
                type="text"
                value={form.businessName}
                onChange={e => set('businessName', e.target.value)}
                placeholder="Juan García — Diseño Web"
                className="input"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">
                <span className="flex items-center gap-1.5"><Hash size={13} />NIF / CIF</span>
              </label>
              <input
                type="text"
                value={form.taxId}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">
                <span className="flex items-center gap-1.5"><MapPin size={13} />Dirección fiscal</span>
              </label>
              <input
                type="text"
                value={form.address}
        ...
        {(form.fullName || form.businessName) && (
          <section className="bg-indigo-50 border border-indigo-100 rounded-2xl p-5">
            <p className="text-xs font-semibold text-indigo-500 uppercase tracking-wider mb-3">Vista previa en facturas</p>
