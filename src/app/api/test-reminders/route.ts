...
export async function GET(request: Request) {
  ...
  for (const invoice of invoices) {
    ...
    if (recentLog) {
      log.push(`${invoice.client_name}: ya enviado recientemente`)
      continue
    }
    ...
    if (!client?.email) {
      log.push(`${invoice.client_name}: sin email`)
      continue
    }
    ...
    const { error: emailError } = await resend.emails.send({
      ...
      html: `
        <div style="font-family:sans-serif;max-width:520px;margin:0 auto;padding:32px;color:#1f2937;">
          <div style="background:#4f46e5;border-radius:8px;padding:6px 10px;display:inline-block;margin-bottom:24px;">
          ...
          <p style="color:#6b7280;font-size:14px;">Si ya realizaste el pago, ignora este mensaje.</p>
          <hr style="border:none;border-top:1px solid #e5e7eb;margin:24px 0;">
          <p style="color:#9ca3af;font-size:12px;margin:0;">Enviado automáticamente por <strong>Cobre</strong></p>
        </div>
      `,
    })
    ...
    if (!emailError) {
      ...
      sent++
      log.push(`${invoice.client_name} (${client.email}): enviado ✓`)
    } else {
      log.push(`${invoice.client_name}: error — ${emailError.message}`)
    }
  }

  return NextResponse.json({ sent, total: invoices.length, log })
}
