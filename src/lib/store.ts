...
export const invoiceStore = {
  ...
  async add(invoice: Invoice) {
    const userId = await getUserId()
    if (!userId) return
    await db().from('invoices').insert({ id: invoice.id, invoice_number: invoice.invoiceNumber || null, user_id: userId, project_id: invoice.projectId || null, client_id: invoice.clientId, client_name: invoice.clientName, project_title: invoice.projectTitle, amount: invoice.amount, items: invoice.items || null, iva_rate: invoice.ivaRate ?? 21, irpf_rate: invoice.irpfRate ?? 0, status: invoice.status, due_date: invoice.dueDate, paid_at: invoice.paidAt || null, created_at: invoice.createdAt })
  },
